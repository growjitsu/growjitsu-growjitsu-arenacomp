  ...
  const fetchAvailableLocations = async () => {
    try {
      ...
      if (availableCountries.length === 0) {
        ...
        if (countryData) {
          const countries = Array.from(new Set(countryData.map(d => d.country.toUpperCase()))).sort();
          setAvailableCountries(countries);
        }
      }

      // Fetch cities (filtered by country if selected)
      ...
      if (filter.country !== 'Todas') {
        cityQuery = cityQuery.ilike('country', filter.country);
      }

      const { data: cityData } = await cityQuery;
      
      if (cityData) {
        const cities = Array.from(new Set(cityData.map(d => d.city.toUpperCase()))).sort();
