...
import { analyzeMedia } from "./src/services/moderationService";
...
async function startServer() {
  ...
  // --- [NÍVEL 0] LOG DE EMERGÊNCIA ABSOLUTO ---
  app.use((req, res, next) => {
    // Este header DEVE aparecer em TODAS as respostas que o Express tocar
    res.setHeader('X-Express-Init', 'active');
    console.log(`[ENTRY] ${req.method} ${req.url}`);
    next();
  });

  // --- [NÍVEL 1] CORS & SECURITY (TOP PRIORITY) ---
  app.use(cors({
    origin: "*",
    methods: ["GET", "POST", "PUT", "DELETE", "OPTIONS"],
    allowedHeaders: ["Content-Type", "Authorization", "X-Requested-With", "X-Express-Init", "X-API-Route", "X-Express-Resolved"],
    exposedHeaders: ["X-Express-Init", "X-API-Route", "X-Express-Resolved"],
    credentials: true,
    maxAge: 86400
  }));
  ...
  const handleAdminResetPassword = async (req: any, res: any) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[ADMIN-AUTH] Falha ao verificar token via Supabase:', e.message);
      }
  ...
  const handleShareRequest = async (req: any, res: any, next: any) => {
    ...
    // Secondary sanitization for id
    if (inferredId) {
      inferredId = inferredId.trim();
    }
    ...
    try {
      if (isHome) {
        ...
        if (!cardData && inferredType && inferredId) {
  ...
  const createPostHandler = async (req: any, res: any) => {
    ...
    try {
      // Basic check for NSFW if media exists
      let moderationStatus: 'approved' | 'blocked' | 'flagged' = 'approved';
      let moderationInfo: any = {};
  ...
  app.get("/api/admin/moderation-queue", async (req, res) => {
    res.setHeader('X-API-Route', 'moderation-queue');
    try {
      const { status = 'flagged,pending,blocked' } = req.query;
      const statuses = (status as string).split(',');

      const { data, error } = await supabaseAdmin
        .from('posts')
        .select('*, profiles(full_name, username, avatar_url, profile_photo)')
  ...
  // 3. Admin Moderation Action
  app.post("/api/admin/moderate-action", async (req, res) => {
    res.setHeader('X-API-Route', 'moderate-action');
    const { post_id, action, reason, admin_id } = req.body;
    ...
    try {
      ...
      await supabaseAdmin.from('moderation_logs').insert({
        ...
        action: action,
        reason: reason,
        details: { previous_status: data.moderation_status }
      });
  ...
  // 4. Block/Suspend User Profile
  app.post("/api/admin/profile-action", async (req, res) => {
    res.setHeader('X-API-Route', 'profile-action');
    const { profile_id, action, reason, admin_id } = req.body;

    if (!profile_id || !action) {
      return res.status(400).json({ error: "Missing profile_id or action" });
    }
    ...
    try {
      ...
      const { error } = await supabaseAdmin
        ...
        .update({
          status: newStatus,
          moderation_notes: reason || "Action by admin"
        })
        .eq('id', profile_id);
